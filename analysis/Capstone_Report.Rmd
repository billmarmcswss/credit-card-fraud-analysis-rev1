...
**Critical Limitation:** The city-to-state mapping process faces inherent challenges due to duplicate city names across states. Many cities share names (e.g., 34 Springfields, 11 Middleburgs across different states), creating ambiguity when mapping based solely on city names. This limitation affects the precision of geographic aggregations and is documented throughout our analysis.
...
## Data Quality Assessment: Geographic Ambiguity

**Challenge Identified:** Many U.S. cities share identical names across different states, creating systematic ambiguity in geographic analysis when only city names are available.

**Impact Assessment:** This analysis quantifies the scope of geographic ambiguity within our dataset to ensure transparent reporting of analytical limitations.

```{r geographic-ambiguity-analysis}
# Identify cities appearing in multiple states
duplicate_cities <- enriched_geo %>%
  count(City, name = "state_count") %>%
  filter(state_count > 1) %>%
  arrange(desc(state_count))

# Calculate impact on original dataset
original_city_counts <- creditCardFraud %>%
  count(Location, name = "transaction_count") %>%
  rename(City = Location)

# Assess ambiguous transactions
ambiguous_transactions <- original_city_counts %>%
  inner_join(duplicate_cities, by = "City")

# Summary statistics
ambiguity_impact <- list(
  total_transactions = nrow(creditCardFraud),
  cities_with_duplicates = nrow(duplicate_cities),
  ambiguous_transaction_count = sum(ambiguous_transactions$transaction_count),
  ambiguous_percentage = round(100 * sum(ambiguous_transactions$transaction_count) / nrow(creditCardFraud), 2)
)

# Create summary table
ambiguity_summary <- data.frame(
  Metric = c("Cities with multiple states", "Ambiguous transactions", "Percentage of dataset", "Geographic confidence"),
  Value = c(
    ambiguity_impact$cities_with_duplicates,
    ambiguity_impact$ambiguous_transaction_count,
    paste0(ambiguity_impact$ambiguous_percentage, "%"),
    paste0(round(100 - ambiguity_impact$ambiguous_percentage, 1), "%")
  )
)

knitr::kable(ambiguity_summary, caption = "Geographic Data Quality Assessment")

# Display most ambiguous cities
if (nrow(duplicate_cities) > 0) {
  cat("Cities with highest geographic ambiguity:\n")
  print(head(duplicate_cities, 10))
}
```

**Professional Note:** In production environments, this limitation would typically be resolved through additional data collection (ZIP codes, coordinates) or external geocoding services. For this analysis, we acknowledge the uncertainty and focus on patterns robust to these limitations.
...
fraud_map <- plot_usmap(data = plot_data, values = "FraudRate", color = "white", size = 0.3) +
  ...
  labs(
    ...
    caption = "Note: Geographic mapping based on city names may contain ambiguities\nMinimum 20 transactions per state for inclusion"
  ) +
  theme_void() +
  theme(
...
**Geographic Mapping Challenges:**
This analysis acknowledges inherent limitations in city-to-state mapping due to duplicate city names across states. Approximately `r ambiguity_impact$ambiguous_percentage`% of transactions involve potentially ambiguous geographic assignments. This limitation is documented for transparency and should be considered when implementing geographically-targeted interventions.

**Future Enhancements:**
- Collection of ZIP codes or coordinates for precise geographic attribution
- Integration with external geocoding services for validation
- Implementation of probabilistic geographic assignment methods
- Enhanced merchant identification protocols to reduce data quality issues

### Professional Standards and Reproducibility

 This analysis employs industry-standard statistical methodologies and transparent documentation to ensure reproducibility and peer